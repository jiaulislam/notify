    ...
    return f"""\
            <html>
                <body style="font-family: Arial, Helvetica, Courier New">
                    <p>Dear <span style="font-weight: bold;">{payload.user_name}</span>,<br>
                    Greetings from Pragati Life Insurance Ltd<br>
                    </p>
                    <p>Your payment couldn't be processed via bKash/Nagad due to a lapsed policy.</p>
...
def next_bill_date(payload: EmailNotifier) -> str:
    return f"""\
            <html>
            <body>
                <p>Dear {payload.user_name},<br>
                Greetings from Pragati Life Insurance Ltd.<br>
                </p>
                <p>Your next paybill generate date is on {payload.bill_activation_date}.</p>
                <p>Please contact your agent for more details</p>
...
def send_mail(payload: EmailPayload):
    try:
        with smtplib.SMTP(
            ...
            body = payload.body
            message["Subject"] = "Lapse Email Test From Pragati Life Insurance"
